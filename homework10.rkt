...
(define =
  (lambda (x y)
    (and (zero? (diff x y)) (zero? (diff y x)))))
;; tests
(test (->bool (= 0 0)) => '#t)
(test (->bool (= 1 1)) => '#t)
...
;; interleave-helper: A (Listof A) -> (Listof (Listof A))
;; a helper function for interleave that
;; consumes an item, an accumulator list
;; and a list, and returns a list of lists where the
;; item is inserted at all possible places in the original list. 
(define/rec interleave-helper
  (lambda (x l r)
    (if (null? r)
        (cons (append l (cons x null)) null)
        (cons (append l (append (cons x null) r))
              (interleave-helper x (append l (cons (car r) null)) (cdr r))))))
;; tests
...
(define/rec unique?
  (lambda (list)
    (or (null? list)
        (and (not (member? (car list) (cdr list))) (unique? (cdr list))))))
;; tests
  ...
  (lambda (edges)
    (with [n (add1 (length edges))]
      (with [assignments (permutations (range n))]
        (filter (lambda (a) (graceful? edges a)) assignments)))))
...
(define hours-spent (+ 5 6))
(test (->nat hours-spent) => '11) ;; stucked at interleave