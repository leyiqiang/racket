...
#| Formal specs for `eval':
     ...
     eval({* E1 E2}) = eval(E1) * eval(E2)
     eval({/ E1 E2}) = eval(E1) / eval(E2)
     eval({sqrt E1}) = sqrt(eval(E1)) (if eval(E1) >= 0)
                     = error! (else)
     eval(id)        = error!
...
(test (run "{with x 1 y}") =error> "bad `with' syntax in")
(test (run "{with {x 5} {x}} ") =error> "bad syntax in")
(test (run "{with {x 1} {* x x}}") => '(1))
(test (run "{with {x 6} {with {y 3} {/ x y}}}") => '(2))
(test (run "{sqrt 9}") => '(3 -3))
